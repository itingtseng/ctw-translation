...
The grid defaults to `Story context order`, which preserves the source sequence and keeps neighboring dialogue together. `Needs context first` moves yellow context-risk rows to the top of the same Excel-like grid without creating a separate section. In the default `Needs context queue`, only yellow rows accept actions while every translation remains visible; selecting `Edit all translations` unlocks every row while retaining yellow highlighting. `Needs context` and `Keep source text` are quick checkbox columns rather than a separate status control.
...
The review grid offers dedicated `Needs context queue`, `QA issue queue`, and `Low confidence queue` modes plus corresponding context-, QA-, and low-confidence-first sort options. A visible legend identifies yellow context-review cells, red QA cells, and blue low-confidence cells, while a `Confidence score` column keeps the evidence-based percentage in the main grid. Above the QA report, a downloadable Context review expander lists every deterministic context risk with its source, confidence, reason, and available provenance. A safe deterministic suggestion, such as restoring terminal punctuation, includes a **Use fix** action that copies it into `Final translation`; semantic or ambiguous problems display `No safe automatic fix available` and require a reviewer edit or a Needs context decision. The original AI output remains unchanged for auditability. Reviewed CSV exports include separate `__qa_issue`, `__suggested_fix`, and `__final_translation` fields.

Context review, Game QA issues, Failure triage, and the localization review grid use the same ordered information schema: String ID, language, speaker, original, AI translation, confidence score and level, context risk, QA issue, suggested fix, failure reason, provenance/details, and final translation. Fields that do not apply remain blank. The workbench additionally keeps its operational fields—Select, Details, Notes, Needs context, and Keep source text—while Failure triage keeps only final translation and resolution note editable.
